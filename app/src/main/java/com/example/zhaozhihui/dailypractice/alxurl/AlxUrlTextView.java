...
/**
 * Created by zhaozhihui on 2018/2/6.
 * 将一部分html带a标签解析出来 截取到href自行跳转
 */
...
public class AlxUrlTextView extends AppCompatTextView {
    ...
    public static Object[][]  getUrlFromJDP(String source){
        ...
        Pattern pattern = Pattern.compile("<a((?!comment).)*?>([^<>]*?[\\u4e00-\\u9fa5]+[^<>]*?)+(?=</a>)");//首先将a标签分离出来
        ...
        while(matcher.find()){
            ...
            String url = raw.substring(firstIndex+1, lastIndex);
            ...
            ends.add(i,matcher.end() + "</a>".length());
